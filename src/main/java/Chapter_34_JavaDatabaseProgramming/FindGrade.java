...
/* Demonstrates connecting to a database from a JavaFX program. The program lets the user enter the SSN and the course ID to find a student’s grade, as
shown in Figure 34.23. The code in Listing 34.2 uses the MySQL database on the localhost. */
...
public class FindGrade extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        btShowGrade.setOnAction(e -> showGrade());          // button listener
    ...
    private void initializeDB() {
        try {
            ...
            Connection connection = DriverManager.getConnection                                 // connect to MySQL database
                    ("jdbc:mysql://localhost/javabook", "scott", "tiger");
            System.out.println("Database connected");

            // Create a statement
            stmt = connection.createStatement();            // execute statement
        }
        catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    private void showGrade() {                              // show result
