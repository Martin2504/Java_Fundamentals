package Chapter_35_AdvancedDatabaseProgramming;
import java.sql.*;
...
/*  Presents a universal SQL client that enables you to connect to any relational database and execute SQL
...
public class SQLClient extends Application {
    ...
    /** Connect to DB */
    private void connectToDB() {                            // Method invoked when the 'connect to database' button is clicked
    ...
    private void executeSQL() {                             // Method invoked when the 'execute SQL command' button is clicked
        if (connection == null) {
            ...
            String[] commands = sqlCommands.replace('\n', ' ').split(";");          // extracts commands seperated by semi colon

            for (String aCommand: commands) {               // determines whether the command is a SELECT query or a DDL or data modification statement
